...
create or replace table betting_dv.football_match_his_l_s_ml_poisson_expected_xg_adapt as
with div_perc_adapt
as
(
select
	...
	avg(div_avg_home_xgoals) over (partition by FEATURE_TYPE, DIVISION order by MATCH_DATE ROWS BETWEEN 100 PRECEDING and 1 PRECEDING) 			div_avg_home_xgoals,
	avg(div_avg_away_xgoals) over (partition by FEATURE_TYPE, DIVISION order by MATCH_DATE ROWS BETWEEN 100 PRECEDING and 1 PRECEDING) 			div_avg_away_xgoals,
	round(local.div_avg_home_xgoals / local.div_avg_home_exp_goals,4) exp_perc_diff_home,
	round(local.div_avg_away_xgoals / local.div_avg_away_exp_goals,4) exp_perc_diff_away,
...
FROM 
	...
	SELECT
		...
		avg(s.HOME_XGOALS)	div_avg_home_xgoals,
		avg(s.AWAY_XGOALS )	div_avg_away_xgoals
	FROM 
...
select
	...
	eg.feature_type || '/expG xG adapted' feature_type,
