...
v1:
	- views:
		- football_match_cur_l_s_eventbased_probs
	- probs for:
...
--create or replace view betting_dv.football_match_cur_l_s_eventbased_probs

--drop table betting_dv.football_match_cur_l_s_eventbased_probs;
...
from
	...
	join
		...
		from
			...
			select
				...
				dense_rank() over (partition by lnk.football_match_cur_lid order by his.match_date desc) direct_rang
			from
				betting_dv.football_match_cur_l lnk
		...
		on (fixtures.football_match_cur_lid = direct_his.football_match_cur_lid)
	...
	join
		...
		from
			...
			select
				...
				--rank for number of games
				dense_rank() over (partition by lnk.football_match_cur_lid order by his.match_date desc) match_rang
			from
				betting_dv.football_match_cur_l lnk
				join betting_dv.football_match_his_l his
					on lnk.football_team_home_hid = his.football_team_home_hid
		...
		on (fixtures.football_match_cur_lid = home_his.football_match_cur_lid)
	...
	join
		...
		on (fixtures.football_match_cur_lid = away_his.football_match_cur_lid)
;