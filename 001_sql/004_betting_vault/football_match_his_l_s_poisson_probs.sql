...
/************************************************
	...
	- probs for Poisson distribution
	
v1:
	- initial
	- views:
		- football_match_cur_l_s_strenght_weakness_odds
	- calculation for following betting markets
		- 1 / X / 2
	- 30 match history
	
v2:
	- changed to column based
	- betting markets droped

v3:
	- name changed
	- based on attack defence strenght satellite
...
--drop view betting_dv.football_match_cur_l_s_poisson_probs;

create or replace table betting_dv.football_match_his_l_s_poisson_probs
as
select
	...
	round(betting_dv.poisson(0, local.home_expect_goals),4) prob_home_0,
	round(betting_dv.poisson(1, local.home_expect_goals),4) prob_home_1,
	round(betting_dv.poisson(2, local.home_expect_goals),4) prob_home_2,
	round(betting_dv.poisson(3, local.home_expect_goals),4) prob_home_3,
	round(betting_dv.poisson(4, local.home_expect_goals),4) prob_home_4,
	round(betting_dv.poisson(5, local.home_expect_goals),4) prob_home_5,
	round(betting_dv.poisson(6, local.home_expect_goals),4) prob_home_6,
	...
	round(betting_dv.poisson(0, local.away_expect_goals),4) prob_away_0,
	round(betting_dv.poisson(1, local.away_expect_goals),4) prob_away_1,
	round(betting_dv.poisson(2, local.away_expect_goals),4) prob_away_2,
	round(betting_dv.poisson(3, local.away_expect_goals),4) prob_away_3,
	round(betting_dv.poisson(4, local.away_expect_goals),4) prob_away_4,
	round(betting_dv.poisson(5, local.away_expect_goals),4) prob_away_5,
	round(betting_dv.poisson(6, local.away_expect_goals),4) prob_away_6,
	...
	, 4) prob_away_win
from
	betting_dv.football_match_his_l_s_attack_defence_strength_30
;